...
pub fn native_random_next_bool(_args: &mut Vec<Value>) -> NativeResult {
    RNG.with(|rng| {
...
pub fn native_random_next_int(_args: &mut Vec<Value>) -> NativeResult {
    RNG.with(|rng| {
        let mut rng = rng.borrow_mut();
...
pub fn native_random_next_int_range(args: &mut Vec<Value>) -> NativeResult {
    if args.len() < 2 {
...
pub fn native_random_next_float(_args: &mut Vec<Value>) -> NativeResult {
    RNG.with(|rng| {
        let mut rng = rng.borrow_mut();
...
pub fn native_random_next_float_range(args: &mut Vec<Value>) -> NativeResult {
    if args.len() < 2 {
